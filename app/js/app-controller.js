...
(function () {
  ...
  $("btn-new-individual").onclick = function () {
    state.mode = "individual"; state.students = [blankStudent()]; state.activeStudent = 0; state.results = null;
  ...
  $("btn-new-group").onclick = function () {
    ...
    state.students = [blankStudent(), blankStudent()]; state.activeStudent = 0; state.results = null;
    clearImportReview(); rebuildAnswerMaps(); refreshAll();
  };
  $("btn-example-individual").onclick = function () {
    state.mode = "individual"; state.students = [deep(EX.individual)]; state.activeStudent = 0; state.results = null;
  ...
  $("btn-example-group").onclick = function () {
    ...
    state.students = g.students; state.activeStudent = 0; state.results = null;
  ...
  $("file-json").onchange = function (ev) {
    readFile(ev.target.files[0], function (text) {
      try {
        ...
        clearImportReview(); state.activeStudent = 0; state.results = null; rebuildAnswerMaps(); refreshAll();
  ...
  $("btn-add-student").onclick = function () {
    ...
    rebuildAnswerMaps(); invalidateGroupAnalysis(); refreshAll();
  };
  function renderRoster() {
    ...
    state.students.forEach(function (s, i) {
      ...
      del.setAttribute("aria-label", "Remove " + (s.student.student_name || "student " + (i + 1)) + " from group");
      del.onclick = function () { removeGroupStudentAt(i); };
      d.appendChild(sel); d.appendChild(del); r.appendChild(d);
    });
    $("g-name").value = state.group.group_name || "";
    $("g-id").value = state.group.group_id || "";
  }
  $("g-name").onchange = function () { state.group.group_name = this.value; invalidateGroupAnalysis(); };
  $("g-id").onchange = function () { state.group.group_id = this.value; invalidateGroupAnalysis(); };
  ...
  ["f-name", "f-id", "f-grade", "f-rw", "f-math", "f-total", "f-target", "f-testdate", "f-weeks", "f-spw", "f-maxsess", "f-format", "f-accom", "f-template", "f-diagdate"].forEach(function (id) {
    $(id).addEventListener("change", function () {
      saveStudentForm();
      if (id === "f-template") { rebuildAnswerMaps(); renderAnswers(); }
      if (state.mode === "group") { invalidateGroupAnalysis(); renderRoster(); }
  ...
  function renderGaps(s) {
    ...
    (s.student.known_foundational_gaps || []).forEach(function (g2, i) {
      ...
      var del = document.createElement("button"); del.className = "btn small danger"; del.textContent = "Remove";
      del.onclick = function () { s.student.known_foundational_gaps.splice(i, 1); if (state.mode === "group") invalidateGroupAnalysis(); else state.results = null; renderGaps(s); };
      row.appendChild(del);
      row.querySelector("[data-k=lesson]").onchange = function () { g2.lesson_id = this.value; if (state.mode === "group") invalidateGroupAnalysis(); else state.results = null; };
      row.querySelector("[data-k=sev]").onchange = function () { g2.severity = this.value; if (state.mode === "group") invalidateGroupAnalysis(); else state.results = null; };
      row.querySelector("[data-k=ev]").onchange = function () { g2.evidence = this.value; if (state.mode === "group") invalidateGroupAnalysis(); else state.results = null; };
      box.appendChild(row);
    });
  }
  $("btn-add-gap").onclick = function () {
    ...
    if (state.mode === "group") invalidateGroupAnalysis(); else state.results = null;
    renderGaps(s);
  };
  ...
  function populatedStudent(student) {
    var s = student || {}, identity = s.student || {}, scores = s.scores || {}, diagnostic = s.diagnostic || {};
    return !!(identity.student_name || identity.student_id || (diagnostic.answers || []).length ||
      scores.reading_writing_scaled != null || scores.math_scaled != null || scores.total_scaled != null);
  }

  function invalidateGroupAnalysis() {
    if (state.mode !== "group") return;
    state.results = null;
    show("panel-results", false);
    window.dispatchEvent(new Event("tp:group-analysis-stale"));
  }

  function groupInputSnapshot() {
    saveStudentForm();
    return deep({
      schema_version: "2.0",
      course_type: "group",
      group_name: state.group.group_name || "Group",
      group_id: state.group.group_id || null,
      students: state.students
    });
  }

  function commitImportedGroupStudent(student, replaceIndex) {
    if (state.mode !== "group") throw new Error("Load or create a group case before importing diagnostic PDFs.");
    var value = deep(student);
    if (replaceIndex != null && replaceIndex >= 0 && replaceIndex < state.students.length) {
      state.students[replaceIndex] = value;
      state.activeStudent = replaceIndex;
    } else {
      var blankIndex = -1;
      for (var i = 0; i < state.students.length; i++) {
        if (!populatedStudent(state.students[i])) { blankIndex = i; break; }
      }
      if (blankIndex >= 0) {
        state.students[blankIndex] = value;
        state.activeStudent = blankIndex;
      } else {
        if (state.students.length >= 20) throw new Error("Groups support at most 20 students.");
        state.students.push(value);
        state.activeStudent = state.students.length - 1;
      }
    }
    rebuildAnswerMaps();
    invalidateGroupAnalysis();
    refreshAll();
  }

  function removeGroupStudentAt(index) {
    if (state.mode !== "group" || index < 0 || index >= state.students.length) return false;
    state.students.splice(index, 1);
    state.answerMaps.splice(index, 1);
    state.activeStudent = Math.max(0, Math.min(state.activeStudent, state.students.length - 1));
    invalidateGroupAnalysis();
    refreshAll();
    return true;
  }
  ...
  function refreshAll() {
    var hasStudent = !!state.mode && state.students.length > 0;
    show("panel-group", state.mode === "group");
    show("panel-student", hasStudent);
    show("panel-answers", hasStudent);
    show("panel-calc", !!state.mode);
    show("panel-results", false);
    if (state.mode === "group") renderRoster();
    if (hasStudent) { loadStudentForm(); renderAnswers(); }
  }

  window.TP_APP_API = {
    getMode: function () { return state.mode; },
    getGroupInput: groupInputSnapshot,
    commitImportedGroupStudent: commitImportedGroupStudent,
    removeGroupStudentAt: removeGroupStudentAt,
    invalidateGroupAnalysis: invalidateGroupAnalysis,
    note: note,
    startNote: startNote
  };
